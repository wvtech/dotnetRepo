			...
			try {
				...
				if (!File.Exists(this.audioFilePath.Path))
				{
					...
					{	
						...
						AD.FileName="ANDROID"+CallingAPI.LoginUserName+CallingAPI.CallToName.ToString();//+Time;
						AD.stream=streaming;
						AD.ReceiverUserId=CallingAPI.CallToId;
						AD.DeviceType="ANDROID";
						//AD.SenderUserId=CallingAPI.UserId;
					}
					else
					{						
						AD.FileName="ANDROID"+CallingAPI.LoginUserName+"Admin";//+Time;
						AD.stream=streaming;
